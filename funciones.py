...
def parsear_nombre(nombre):
    """Procesa la cadena para que pueda ser un nombre en un fichero de almacenamiento

    Args:
        nombre (str): cadena de texto que se procesa

    Returns:
        str: devuelve la cadena modificada.
    """
    reemplazos = {
        ":": "a",
        "<": "b",
...
def copiar_a_training_file(tipo, file:str):
    ...
    with open(file) as file:
        ...
        copiar_a_training_lines(tipo,lineas=lineas)
...
def copiar_a_training_deteccion(path_carpeta:str):
    ...
    def copiar_a_training(lineas):
        ...
        if (len(lineas) >= 3):
            ...
            warning(f"No hay sufucientes imagenes\nImagenes:\n")
    ...
    for archivo in archivos:
        if archivo.endswith(('.png', '.jpg', '.jpeg')):  # Agrega otros formatos de imagen si es necesario
            ...
            if nombre_base_imagen not in nombres_base_txt:
                ...
                info(f"Eliminada: {archivo}")
    
    info(f"Total de imágenes eliminadas: {imagenes_eliminadas}")
...
def copiar_a_training(path_carpeta:str):
    ...
    return: true en caso de que haya imagenes en las carpetas de entrenamiento
    """
    ...
    for carpeta in carpetas:
        copiar_a_training_lines(os.path.split(carpeta)[1],encontrar_imagenes(carpeta,extensions=['.webp','.jpg']))
    
    
    if (not os.listdir(RUTA_DESTINO_TRAINING)):
        info("carpeta vacia")
        
    return os.listdir(RUTA_DESTINO_TRAINING)

def copiar_a_training_lines(tipo:str, lineas,txt_asociado=False):
    """Copia las imagenes a la carpeta de entrenamiento
    ...
    if (len(lineas) >= 3):
        ...
        for i in range(num_valid):
            copiar_archivo(lineas[i], os.path.join(ruta_training_data["valid"],tipo),txt_asociado)
        
        for i in range(num_valid,num_valid + num_test):
            copiar_archivo(lineas[i], os.path.join(ruta_training_data["test"],tipo),txt_asociado)
            
        for i in range(num_valid + num_test, len(lineas)):
            copiar_archivo(lineas[i], os.path.join(ruta_training_data["train"],tipo),txt_asociado)
    else:
        warning(f"No hay sufucientes imagenes\nImagenes:\n")
        if VERBOSE:
            for linea in lineas:
...
def copiar_archivo(ruta_origen, ruta_destino, txt_asociado: bool = False):
    ...
    if txt_asociado:
        ruta_origen = str(ruta_origen).split(".")[0] + ".txt"
        shutil.copy(ruta_origen, os.path.join(ruta_destino,obtener_GBIF(ruta_origen)))
...
def convert_to_webp(input_image_path: str, output_image_path = "", quality=100, remove_original=True, only_rescal = False):
    ...
    try:
        ...
        warning(f"Imagen no identificable: {input_image_path}, se procede a eliminar.")
        os.remove(input_image_path)

    except FileNotFoundError:
        warning(f"Archivo no encontrado: {input_image_path}")

    except Exception as e:
        warning(f"Se ha producido una excepción con la imagen: {input_image_path}\nError: {e}")
...
def shuffle_DataFrame(df: pd.DataFrame):
    """Esta función desordena un dataframe para que sea aleatorio y lo guarda en un csv designado en CSV_DATOS

    Args:
        df (List DataFrame): lista de DataFrames que hay que desordenar.
        
    return: devuelve una nueva lista de chunks con los ficheros desordenados
    """
    ...
    info(f"Archivo desordenado guardado como: {CSV_DATOS}")
    
    return chunks_list


def recortar_imagenes(src_img: str, model: YOLO = None, delete_original: bool = True):
    """
    ...
    if image is None:
        info(f"Error al leer la imagen: {src_img}")
        return []
    ...
    for result in results:
        ...
        for box in boxes:
            ...
            if descartar_imagen_mala(cropped_img_path):
                ...
                info(f'Imagen recortada guardada en: {cropped_img_path}')
